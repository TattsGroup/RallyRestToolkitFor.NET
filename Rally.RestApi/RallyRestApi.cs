...
namespace Rally.RestApi
{
	...
	public class RallyRestApi
	{
		...
		/// <param name="maxRetries">Requests will be attempted a number of times (defaults to 2)</param>
		...
		/// </code>
		/// </example>
		public RallyRestApi(ApiAuthManager authManger = null, string webServiceVersion = DEFAULT_WSAPI_VERSION, int maxRetries = 2, TraceFieldEnum traceInfo = RallyRestApi.DEFAULT_TRACE_FIELDS)
		{
		...
		private string GetSecurityToken()
		{
			try
			{
				DynamicJsonObject securityTokenResponse = DoGet(new Uri(GetFullyQualifiedRef(SECURITY_ENDPOINT)), false);
